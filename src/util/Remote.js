...
class Remote {
     ...
     * @date 2019-05-13
     */
    open = (url, data, option = {
        urlType: 'default',
    }) => {
        let sendURL = `${this.genDomainForEnv(option.urlType)}${url}`;
        const dealData = {};
        ...
        sendURL += queryData;
        console.log('导出url', encodeURI(sendURL));
    ...
    get = (url, data, option = {
        ...
        let isShowFailToast = true;
        if (option && option.isShowFailToast === false) {
        ...
        if (method === Remote.METHOD.GET) {
            ...
            switch (type) {
                case 'json':
                    ...
                    _forEach(data, (val, key) => {
                        // 和后端协商后的产物，碰到空字符串和undefined则不传此字段，
                        // 如需要传递空字符串来表示修改的部分可和后端协商传 ' ' 或 -1 等数据来表示
    ...
    genDomainForEnv = (type) => {
        ...
        const typeJson = {
            ...
            report: `${Config[env].apiUrl}${Config[env].reportUrlFilter}`,
            doc: `${Config[env].apiUrl}${Config[env].docUrlFilter}`,
            h5: `${Config[env].targetH5Url}`,
        };
...
export default {
    ...
    genDomainForEnv: remote.genDomainForEnv,
};
